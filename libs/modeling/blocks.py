...
class LocalMaskedMHCA(nn.Module):
    ...
    def __init__(
        ...
        self.n_embd = n_embd
        self.n_head = n_head
        print("LocalMaskedMHCA's nhead is: ", self.n_head)
        self.n_channels = n_embd // n_head
        print("this is a channel for head:", self.n_channels)
        self.scale = 1.0 / math.sqrt(self.n_channels)
...
class TransformerBlock(nn.Module):
    ...
    # ! series for cascade attention
    def forward(self, x, mask, pos_embd=None):
        # ------------------------------ global attention for channel------------------------------
        out, out_mask = self.attn_channel(self.ln1_channel(x), mask)
        out_mask_float = out_mask.to(out.dtype)
        out = self.pool_skip_channel(x) * out_mask_float + self.drop_path_attn_channel(
            out
        )
        # FFN
        out = out + self.drop_path_mlp_channel(
            self.mlp(self.ln2_channel(out)) * out_mask_float
        )
        # optionally add pos_embd to the output
        if pos_embd is not None:
            out += pos_embd * out_mask_float

        # ------------------------------ global attention for time------------------------------
        x, mask = out, out_mask
